...
static void handle_new_connection(SOCKET server_socket, fd_set *reads);
...
int main(void) {
    ...
    */
    ...
    FD_SET(server_socket, &reads); // server_socket을 감시 대상으로 등록 (클라이언트의 연결 요청 감시)
    ...
    while (1) { // 서버를 계속 실행 — select → 이벤트 처리를 반복
        ...
        */
        ...
        for (int i = 0; i < reads.fd_count; i++) {
            ...
            // server_socket에 이벤트 발생: 새로운 클라이언트의 연결 요청
            if (reads.fd_array[i] == server_socket) {
                handle_new_connection(server_socket, &reads);
...
// server_socket에 이벤트 발생: 새로운 클라이언트의 연결 요청 처리
static void handle_new_connection(SOCKET server_socket, fd_set *reads) {
    SOCKADDR_IN clnt_adr;
    int adr_sz = sizeof(clnt_adr);
    SOCKET clnt_sock = accept(server_socket, (SOCKADDR*)&clnt_adr, &adr_sz);
