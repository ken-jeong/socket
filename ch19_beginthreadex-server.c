...
unsigned WINAPI process_client(LPVOID arg) {
    ...
    while (is_running) {
        ...
        while (received_bytes < total_bytes) {
            int result = recv(client_socket, &message[received_bytes], total_bytes - received_bytes, 0);
            if (result <= 0) {
                printf("<ERROR> recv() failed or connection closed.\n");
                is_running = 0;
                break;
            } else {
                received_bytes = received_bytes + result;
                printf("Thread> Received %d bytes. sum=%d, total=%d\n", result, received_bytes, total_bytes);
            }
        }
